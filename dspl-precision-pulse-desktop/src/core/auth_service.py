...
class AuthService(QObject):
    ...
    def get_token(self) -> Optional[str]:
        ...
        if not hasattr(self, '_jwt_token') or not self._jwt_token:
            try:
                import requests
                response = requests.post(
                    'http://localhost:5000/api/auth/login',
                    json={'email': self.current_user['email'], 'password': getattr(self, '_password', '')},
                    timeout=5
                )
        ...
        return self._jwt_token if hasattr(self, '_jwt_token') else None
